"""Render the approved profile into GitHub-compatible linked SVG assets.

No font files, credentials or fabricated statistics are published. Static
components are rendered from the approved layout. Typing and the contribution
snake remain animated; reduced-motion readers receive separate static assets.
"""
...
def download(url: str) -> bytes:
    for attempt in range(3):
        try:
            with urlopen(Request(url,headers={'User-Agent':'lora-profile/5','Accept':'image/svg+xml,image/*,*/*'}),timeout=40) as r:
...
def validate_svg(data: bytes, kind: str) -> str:
    ...
    if kind=='overview' and not re.search(r'rank',data.decode(),re.I):
        raise ValueError('Overview rank is missing')
    if kind=='languages' and not re.search(r'%|language',words,re.I):
        raise ValueError('Language data missing')
    return words
...
def raster_svg(name: str, data: bytes, css_width: float, title: str, pad=0):
    ...
    save_svg(name,f'<svg xmlns="http://www.w3.org/2000/svg" width="{css_width+pad:.2f}" height="{height+pad:.2f}" viewBox="0 0 {css_width+pad:.2f} {height+pad:.2f}" role="img" aria-label="{html.escape(title,quote=True)}"><title>{html.escape(title)}</title><image x="{pad/2}" y="0" width="{css_width:.2f}" height="{height:.2f}" href="{uri(buf.getvalue(),"image/webp")}"/></svg>')
...
def typing_svg(page,theme,mode,static=False):
    ...
    total=offset;css='text{font-family:system-ui,"Noto Sans CJK SC","Microsoft YaHei",sans-serif;letter-spacing:.01em;font-weight:650;}'
    body=''
...
def snake_svg(theme,mode,static=False):
    ...
    style='font-family:system-ui,"Noto Sans CJK SC","Microsoft YaHei",sans-serif'
    save_svg(f'snake-{theme}-{mode}'+('-static' if static else ''),f'<svg xmlns="http://www.w3.org/2000/svg" width="{w}" height="{h:.2f}" viewBox="0 0 {w} {h:.2f}" role="img"><title>lora-sys 的真实 GitHub 贡献贪吃蛇</title><rect x=".5" y=".5" width="{w-1}" height="{h-1:.2f}" rx="12" fill="#{p["bg"]}" stroke="#{p["line"]}"/><g style="{style}"><text x="{margin}" y="34" fill="#{p["fg"]}" font-size="17" font-weight="650">贡献贪吃蛇</text><text x="{margin}" y="{h-19:.2f}" fill="#{p["muted"]}" font-size="11">GitHub Actions 每日生成 · Platane/snk</text></g><image x="{margin}" y="56" width="{draw_w}" height="{dh:.2f}" href="{uri(data)}"/></svg>')
...
def build(local=False):
    ...
    with sync_playwright() as p:
        exe='/usr/bin/chromium' if local else None
        ...
        for theme in PALETTES:
            for mode,(viewport,full,cw,tw) in SIZES.items():
                ...
                for name,sel,kind in SPECS:
                    ...
                    data=el.screenshot(animations='disabled',omit_background=True)
                    raster_svg(f'{name}-{theme}-{mode}',data,cw if kind=='card' else full,title,pad=8 if kind=='card' else 0)
                for idx in range(8):
...
def picture(name,alt=None,link=None,static=False):
    ...
    def src(media,theme,mode,still=False):
        fn=f'{name}-{theme}-{mode}'+('-static' if still else '')+'.svg'
        return f'<source media="{media}" srcset="{RAW+fn}">'
...
def make_readme():
    ...
    md='''<!-- Lora approved profile v5. Source and asset provenance: assets/profile and output/v5/sources.json. -->
<a id="top"></a>
...
'''
    ...
    md+=group(['article-budget','article-tau','article-harness'])
...
if __name__=='__main__':
    a=argparse.ArgumentParser();a.add_argument('--local',action='store_true');args=a.parse_args();build(args.local)
